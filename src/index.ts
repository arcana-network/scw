...
import { custom, createPublicClient, createWalletClient, defineChain, parseAbi, http } from "viem"
...
export class SCW {
  ...
  if (params.provider == undefined) {
    if (params.private_key == undefined) {
      throw new Error("Private key not provided");
    }
    this.provider = createPublicClient({
      transport: http(params.rpc_url),
    });
  } else {
    this.provider = createPublicClient({
  ...
  if (params.arcana_key.includes("xar")) {
    let [xar, env, key] = params.arcana_key.split("_");
    this.arcana_key = key;
    if (env == "dev") {
  ...
  if (params.gateway_url != undefined) {
    // check if gateway url ends with / if yes then remove it
    if (params.gateway_url.endsWith("/")) {
      params.gateway_url = params.gateway_url.slice(0, -1);
    }
    this.gateway_url = params.gateway_url;
  }
  ...
  if (params.provider == undefined) {
    const account = privateKeyToAccount(params.private_key as Hex);
    ...
    const [account] = await params.provider.request({ method: 'eth_requestAccounts' })
    this.wallet = createWalletClient({
      account,
      chain: this.chain,
      transport: custom(params.provider)
    })
    this.account_address = account;
  }

  if (accountType == "scw") {
    this.pre_scw = true;
    this.scwAddress = this.account_address;
    return;
  } else {
    this.pre_scw = false;
  }

  this.smart_account_owner = this.account_address;
